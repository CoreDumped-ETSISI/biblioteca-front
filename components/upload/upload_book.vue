        ...
        methods: {
            getBookFile(event) {
                if (event !== undefined && event.target.files.length > 0) {
                    ...
                    const filename = file.name.substring(0, file.name.length - (fileExtension.length + 1));
...
<template>
    <div class="upload-container">
        <popupMessage v-on:finished="popupExit" v-if="popup.show" v-bind:popup="popup"></popupMessage>
        <div class="upload-inner">
            <div class="upload-text">Sube un libro</div> 
            <div class="upload-box">
                <input @change="getBookFile" type="file" name="" id="book-file" accept=".pdf, .epub"/>
                <i class="material-icons">{{this.validBook ? 'book' : 'cloud_upload'}}</i>
